...
revisions:
  - 2026-09-04 §9.3 포트폴리오 개수 재갱신 — 실물 맞춤 H25(`zone-quota-exchange-fill`) 편입으로 24개 → 25개. 규칙 무변경
...
흐름은 "초기해 생성 → ALNS 개선"이 전부다.

**폐기된 것과 재량인 것을 구분한다 (2026-09-02 정밀화).** 이전 설계의
"초기해 ≤8 → **phase-1 screen** → phase-2"에서 폐기된 것은 **근사 점수로 후보를 거르는
2단계 파이프라인**이다 — 근사로 수락·선정을 결정하는 것이 §9.2 MUST NOT에 걸리기 때문이다.
...
결정적 construction 25개(기본 8 + 확장 14 + 실물 맞춤 3) 포트폴리오를 쓴다
