...
- **DP**는 "남은 유형별 대수 벡터"를 상태로 두고 존을 하나씩 소비하며, 존마다 (a) 그 존을 딱 덮는 최소 조합 (b) 한 대 모자란
  최대 조합 (c) 아무것도 안 줌, 이 세 갈래(프론티어[^frontier])만 전이한다. 값은 **(총 부족, 총 사용 대수, 총 낭비, 총 결손)** 사전식 최소
  (2026-09-05 개정 — 종전 (부족, 낭비, 대수). 결손 = 존마다 max(0, 대수 × 평균 단품 − 낭비), "차 한 대마다 평균 단품 하나가 더 들어갈
  여유"의 부족분. [설계안](../implementation/stage-04-zone-value-function.md)).
...
| **H23 `zone-quota-balanced-fill`** | **0** | 31 | 4,198,408 (2026-09-05 값 함수 개정 후 4,194,052, T52 확정) | 1,002,069 (개정 후 1,004,144) | 507 ms |
