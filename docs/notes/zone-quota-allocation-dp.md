...
> 2026-09-06부터 이 나열은 **존마다 한 번**만 한다(`ZoneFrontier`) — 층의 모든 상태를 덮는 상자 B_t = min(대수_t, maxNeed)로
> 열거해 두고, 상태마다 s ≤ R인 것을 사전식 배열에서 뽑는다. 가지치기가 s에만 의존해 방문 집합이 상자에 단조라 상태마다 따로
> 열거한 것과 같은 집합이 나온다((b)는 확장 마스크로 R 기준 재판정). 실물 ZONE_29에서 2,348,844잎 → 1,933잎.
> 아래 §6.1·§6.2의 표는 "상태 하나의 R로 열거했을 때"의 설명이고 결과 집합은 같다 —
> [설계안](../implementation/stage-04-zone-quota-frontier-budget.md).
...
S = 25,920, 유형 6종이고, H23 전체 507 ms 중 DP는 그 일부다. 2026-09-06부터 열거는 존당 1회(잎 ≤ `MAX_FRONTIER_LEAVES`)이고
상태당 후보 ≤ `MAX_FRONTIER_CANDIDATES`(1,024)라 전이 수가 S × 1,024로 묶인다 — 실물은 상태당 최대 526이라 닿지 않는다.
