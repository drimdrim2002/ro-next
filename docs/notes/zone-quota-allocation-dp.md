...
flowchart LR
    ...
    F --> L["⑤ 층별 DP<br/>상태 = 남은 대수 벡터<br/>값 = (부족, 낭비, 대수)"]
    L --> B["⑥ 최선 상태에서 역추적"]
...
**최적화 대상**: (총 부족 부피, 총 낭비 부피, 총 사용 대수)를 사전식으로 최소화한다 — 부족이 0인 배정 중 낭비가 가장 적고 차량을 가장 덜 쓰는 것.
...
사전식 비교에서 첫 축(부족)이 13,000 vs 0이므로 DP가 이긴다. 실물 fixture에서도 같은 현상이 크게 나타났다 — 부피 수준 근사 잣대로
...
| **값**은 무엇인가 | (Σ부족, Σ낭비, Σ사용 대수) — 작을수록 좋고 사전식 | `long[] value = {…}` |

```text
f₀(전 대수) = (0, 0, 0)
f_{z+1}(r − s) = min_사전식 { f_z(r) + (부족(z, s), 낭비(z, s), Σs) : s ∈ 프론티어(r, z) }
...
flowchart LR
    ...
    subgraph L1["층 1 — 존 A 처리 후"]
        B21["(2,1)<br/>(18000,0,0)"]
        B20["(2,0)<br/>(0,3000,1)"]
        B11["(1,1)<br/>(8000,0,1)"]
        B01["(0,1)<br/>(0,2000,2)"]
    end
    subgraph L2["층 2 — 존 C 처리 후"]
        C21["(2,1)<br/>(31000,0,0)"]
        C20["(2,0)<br/>(13000,3000,1)"]
        C11["(1,1)<br/>(21000,0,1)"]
        C10["(1,0)<br/>(3000,3000,2)"]
        C01["(0,1)<br/>(11000,0,2)"]
        C00["(0,0)<br/>(0,10000,3) ★"]
    end
    A21 -- "s=(0,0) c" --> B21
    A21 == "s=(0,1) a  M×1" ==> B20
    A21 -- "s=(1,0) b" --> B11
...
시작 상태 (2,1), 값 (0,0,0). 프론티어(2,1) = {(0,0), (0,1), (1,0), (2,0)}.

| s | 부류 | capacity | covers | 부족 | 낭비 | 대수 | 도착 상태 | 층 1 값 |
|---|:-:|---:|:-:|---:|---:|---:|---|---|
| (0,0) | c | 0 | ✗ | 18,000 | 0 | 0 | (2,1) | (18,000, 0, 0) |
| (0,1) | a | 21,000 | ✓ | 0 | 3,000 | 1 | (2,0) | (0, 3,000, 1) |
| (1,0) | b | 10,000 | ✗ | 8,000 | 0 | 1 | (1,1) | (8,000, 0, 1) |
| (2,0) | a | 20,000 | ✓ | 0 | 2,000 | 2 | (0,1) | (0, 2,000, 2) |

층 1에서 (1,0)·(0,0)은 도달 불가(null)다.

### 7.3 층 1 → 층 2 (존 C: c1 8,000 S만 · c2 5,000)

상태는 인덱스 오름차순(§4.1 표)으로 순회한다: (2,0)=2 → (0,1)=3 → (1,1)=4 → (2,1)=5.

| from | 기본값 | s | 부류 | capacity | covers | +(부족, 낭비, 대수) | to | 후보 값 | 층 2 결과 |
|---|---|---|:-:|---:|:-:|---|---|---|---|
| (2,0) | (0,3000,1) | (0,0) | c | 0 | ✗ | (13,000, 0, 0) | (2,0) | (13,000, 3,000, 1) | **채택** |
| | | (1,0) | b | 10,000 | ✗ | (3,000, 0, 1) | (1,0) | (3,000, 3,000, 2) | **채택** |
| | | (2,0) | a | 20,000 | ✓ | (0, 7,000, 2) | (0,0) | **(0, 10,000, 3)** | **채택 ★** |
| (0,1) | (0,2000,2) | (0,0) | c | 0 | ✗ | (13,000, 0, 0) | (0,1) | (13,000, 2,000, 2) | 채택(임시) |
| (1,1) | (8000,0,1) | (0,0) | c | 0 | ✗ | (13,000, 0, 0) | (1,1) | (21,000, 0, 1) | **채택** |
| | | (0,1) | b | 21,000 | ✗ | (1, 0, 1) | (1,0) | (8,001, 0, 2) | 기각 — (3,000, …)가 더 좋음 |
| | | (1,0) | b | 10,000 | ✗ | (3,000, 0, 1) | (0,1) | (11,000, 0, 2) | **채택** — (13,000, …)를 밀어냄 |
| | | (1,1) | a | 31,000 | ✓ | (0, 18,000, 2) | (0,0) | (8,000, 18,000, 3) | 기각 — (0, 10,000, 3)가 더 좋음 |
| (2,1) | (18000,0,0) | (0,0) | c | 0 | ✗ | (13,000, 0, 0) | (2,1) | (31,000, 0, 0) | **채택** |
| | | (0,1) | b | 21,000 | ✗ | (1, 0, 1) | (2,0) | (18,001, 0, 1) | 기각 |
| | | (1,0) | b | 10,000 | ✗ | (3,000, 0, 1) | (1,1) | (21,000, 0, 1) | **동률** → 먼저 계산된 값 유지 |
| | | (1,1) | a | 31,000 | ✓ | (0, 18,000, 2) | (1,0) | (18,000, 18,000, 2) | 기각 |
| | | (2,0) | a | 20,000 | ✓ | (0, 7,000, 2) | (0,1) | (18,000, 7,000, 2) | 기각 |

눈여겨볼 행:

- **(1,1)에서 s=(0,1)**: M 한 대(21,000)는 부피로는 13,000을 넘지만 `covers`가 ✗라(Hall) 부족이 `max(1, 13,000 − 21,000) = 1`이 된다.
  "못 덮은 존은 최소 1의 부족"이라는 규칙이 여기서 작동한다.
- **(2,1)에서 s=(1,0) → (1,1)**: (1,1)에서 온 (21,000, 0, 1)과 정확히 동률이다. `compare(...) < 0`일 때만 교체하므로 **먼저 계산된 쪽**
...
|---|---:|---|---|
| (0,0) | 0 | **(0, 10,000, 3)** | 부족 0 — 유일한 완전 덮개 |
| (1,0) | 1 | (3,000, 3,000, 2) | |
| (2,0) | 2 | (13,000, 3,000, 1) | |
| (0,1) | 3 | (11,000, 0, 2) | |
| (1,1) | 4 | (21,000, 0, 1) | |
| (2,1) | 5 | (31,000, 0, 0) | 아무것도 안 줌 |
...
| 전이 동률 | **먼저 계산된 값 유지** (`compare < 0`일 때만 교체) |
