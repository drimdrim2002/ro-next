...
stage: 4
date: 2026-09-04
status: **폐기 결정 (2026-09-05) — 실측 근거 §8. 코드 미편입.** 구현·실측은 2026-09-04에 했고
  §8.1~§8.3의 수치가 폐기의 근거다. 계약 개정과 코드는 2026-09-05에 되돌렸다 —
  포트폴리오는 **24개**로 남는다. 이 문서는 "왜 안 넣었는가"의 기록으로 존치한다
...
revisions:
  ...
  - 2026-09-05 **폐기 결정(사용자)** — §8.3의 "H25 ≤ H3" 행대로. 코드(`ZoneQuotaExchangeFillConstruction`·
    T53~T56)와 계약 개정(§9 목록)을 되돌려 포트폴리오를 24개로 복귀시켰다. **§8 실측 표는 그대로 둔다** —
    그 수치가 폐기의 근거다. `ZoneQuotaBalancedFillConstruction.fill` 추출만 남는다(동작 무변경)
...
(**폐기 뒤**: T53~T56은 삭제됐고 계약 문면은 24개로 되돌아갔다. 이 절은 당시의 설계 기록이다.)
...
> **이 표는 2026-09-04에 전부 적용됐다가 2026-09-05 폐기 결정으로 전부 되돌려졌다.**
> 지금 계약 문서들은 **24개**를 말하고 그것이 맞다 — 아래 목록을 다시 적용하지 마라.
> 아래는 "무엇을 건드렸다가 되돌렸는가"의 기록이고, 남은 것은
> `ZoneQuotaBalancedFillConstruction.fill` 추출 하나뿐이다(동작 무변경).
