...
| **E4** | 초기해 포트폴리오 **병렬 실행** | 8 → 4 축소 확정 (Stage 8 실측 후 사용자 결정) | **소~중** — 기법은 그대로, 실행기와 스레드 설정만 |
...
초기해 construction 8개를 **여러 스레드에서 동시에** 돌리는 실행 구조.
...
**지금 넣는 비용과 나중에 넣는 비용이 같다.** 8개는 서로를 참조하지 않고
...
8개 중 어느 4개를 남길지가 아직 미정이고(그 결정의 입력이 T25의 기법별 실측이다)
...
3. **heuristics 문서 §6·§9** — 실행 루프와 "하지 않는 것" 표의 해당 행.
   기법 8개(§5)와 선택 규칙(§6-4 동률 시 우선순위)은 **손대지 않는다.**
