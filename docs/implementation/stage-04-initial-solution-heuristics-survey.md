...
기각 사유, 그리고 22 → 4 축소를 위한 비교 프로토콜. 같은 규정을 두 곳에 적지 않는다: 의사코드·기권
...
[채택 14개 → heuristics 문서 §5 H9~H22]      [기각·보류 ~30개 → §4 사유]
        │
        ▼
[T25 실측 + Stage 8 → 22개 중 4개만 남긴다 (§5 프로토콜, 사용자 결정)]
```
...
쓰되, 기법이 22개가 되어 재료의 형식을 정한다. 이 절은 **재료를 정하는 것이지 결과를 예단하지 않는다.**
...
1. 강한 LS와 분 단위 예산·차량 무제한 환경에서는 construction이 무의미하다 (EURO-NeurIPS 2022 OptiML —
   ...
   지우기 어려운 차이가 남는다. 그래서 22개를 다 재 보는 것이 맞고, 그 다음에 자른다.
...
| 순위 | 22개 중 `Scores.compare` 순위 | T25 계산 |
