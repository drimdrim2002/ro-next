...
supersedes: version 2.1-review의 25 RESOLVED + 1 OPEN + 2 DEFERRED 상태
related_decisions:
...
| <a id="q-infra-01"></a>`Q-INFRA-01` | RESOLVED | 실제 provider/product/deployment topology는 무엇인가? | 선택된 target/reference runtime은 **AWS S3 (object storage) + AWS Step Functions (durable orchestration) + AWS Lambda (API/coordinator/worker compute)**다. 이 선택은 deployment/IaC·resource sizing 또는 solver 구현 완료를 뜻하지 않는다. SDK와 provider DTO는 AWS adapter/deployment 경계에만 두며, logical port·domain 의미·worker/round completion·retry identity·candidate/result verification·publication 의미는 변경하지 않는다. ECS, GCP, Kubernetes 등은 parity와 별도 승인 뒤의 미래 대체 후보이다. | **2026-07-26 사용자 승인 결정** / Application·Platform·Product. 이 등록부와 Master·Architecture 문서에 같은 변경 단위로 반영함 | 선택된 AWS target의 adapter/deployment 구현은 `RM-8` semantic parity, two-gate publication, shadow/cutover/rollback evidence를 통과해야 함. `Q-BENCH-02` 수치 gate와는 독립 | [Master §1·§4·§15.10·§16.3](master-design.md#1-이-문서의-목적-독자와-사용법), [Architecture §13·§15](architecture-design.md#13-physical-topology와-provider-boundary) |
...
| 상태 | 수량 | 현재 의미 |
|---|---:|---|
| RESOLVED | 26 | `Q-ALG-01`의 후속 사용자 결정, `Q-ALG-02`의 `RESOLVED — KEEP_COW`, 2026-07-26의 `Q-INFRA-01` AWS target 승인 포함 |
| OPEN — EXPERIMENT_REQUIRED | 1 | `Q-BENCH-02`; protocol 확정, 공식 수치 미확정 |
| DEFERRED | 1 | `Q-VAR-01`만 해당; 질문·활성화 금지, resume evidence와 별도 승인 필요 |
...
- `Q-INFRA-01`의 AWS target 선택은 완료됐지만 `Q-BENCH-02`의 공식 수치, `RM-8` parity/cutover evidence, 비용·성능 sizing과 미래 대체 provider 채택은 별도 gate다.
- `Q-VAR-01`만 계속 `DEFERRED`이며 위 실험 항목과 연결해 활성화하지 않는다.

## 5. 영향 문서

- [Master Design](master-design.md): current Java/GCP placeholder와 AWS target inventory, `C-20`, logical-port 경계, `RM-8`, deferred criteria와 질문 상태를 갱신한다.
- [Architecture Design](architecture-design.md): AWS provider mapping, adapter/deployment isolation, AWS parity gate와 미래 substitution 경계를 갱신한다.
- [Architecture-domain implementation design](architecture-domain-implementation-design.md): AWS target/reference phase와 future substitution/ADR 표현을 갱신한다.
- [문서 지도](README.md)와 [2026-07-26 이전 초안](2026-07-26-master-design.md): canonical authority와 historical GCP evidence의 위치를 갱신한다.
